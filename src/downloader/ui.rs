                        ...
                        for (id, url, state) in tasks {
                            match state {
                                ...
                                DownloadState::Downloading(downloaded, total) => {
                                    ...
                                    total_sum += total;
                                    let model: Rc<VecModel<StandardListViewItem>> = Rc::from(VecModel::from(vec![StandardListViewItem::from(id.to_shared_string()), StandardListViewItem::from(url.to_shared_string()), StandardListViewItem::from((downloaded / total * 100).to_shared_string()), StandardListViewItem::from(""), StandardListViewItem::from("")]));
                                    let row: ModelRc<StandardListViewItem> = ModelRc::from(model);
                                    ui_tasks_in_process.push(row);
                                },
