...
impl Account {
    pub async fn new(device_code: &str) -> Option<Self> {
        ...
        let ms_res = client.post("https://login.microsoftonline.com/consumers/oauth2/v2.0/token")
            .form(&params)
            .send().await.ok()?;
    ...
    async fn login(&mut self, ms_token: &str) -> Option<()> {
        ...
        let xbox_res = client.post("https://user.auth.xboxlive.com/user/authenticate")
            .json(&xbox_send_json)
            .send().await.ok()?;
